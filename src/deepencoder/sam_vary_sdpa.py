...
# This class and its supporting functions below lightly adapted from the ViTDet backbone:
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_rel_pos:
            ...
            x = sdp_attention(q, k, v, attn_mask=attn_bias)
        else:
            x = sdp_attention(q, k, v, attn_mask=None)
...
def get_rel_pos(q_size: int, k_size: int, rel_pos: torch.Tensor) -> torch.Tensor:
    ...
    # Scale coords with short length if shapes for q and k are different.
...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        if any(k.startswith("image_encoder.") for k in state_dict):
            mapped = {k[len("image_encoder."):]: v
                      for k, v in state_dict.items()
                      if k.startswith("image_encoder.")}
            missing, unexpected = image_encoder.load_state_dict(mapped, strict=False)
            print("[SAM] Loaded official SAM weights (image_encoder.*).")
            if missing:    print("[SAM] Missing:", missing)
            if unexpected: print("[SAM] Unexpected:", unexpected)

        # (B) Custom mm checkpoint: keys like "vision_tower_high.*"
        elif any(k.startswith("vision_tower_high.") for k in state_dict):
            mapped = {k[len("vision_tower_high."):]: v
                      for k, v in state_dict.items()
                      if k.startswith("vision_tower_high.")}
            image_encoder.load_state_dict(mapped, strict=True)
            print("[SAM] Loaded custom weights (vision_tower_high.*).")

        # (C) As-is
        else:
            missing, unexpected = image_encoder.load_state_dict(state_dict, strict=False)
            print("[SAM] Loaded weights directly (no prefix).")
            if missing:    print("[SAM] Missing:", missing)
            if unexpected: print("[SAM] Unexpected:", unexpected)
